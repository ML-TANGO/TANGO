...
def start(request):
    """
    API for project manager having autonn_cl start
    """
    params = request.query_params
    userid = params.get('user_id')
    project_id = params.get('project_id')

    print(f"[AutoNN_CL] START API 수신 - User: {userid}, Project: {project_id}")
    print(f"[AutoNN_CL] 요청 파라미터: {dict(params)}")

    synchronize_project_manager_db(userid)

    try:
        info = Info.objects.get(userid=userid, project_id=project_id)
        print(f"\n[AutoNN_CL GET/start] Process exists with status [{info.status}]")
        info.print()

        if info.best_net and os.path.isfile(info.best_net):
            print(f"[AutoNN_CL GET/start] Best model exists: {info.best_net}")
    ...
    try:
        if str(info.process_id) in PROCESSES:
            print(f"[AutoNN_CL GET/start] Terminating previous process #{info.process_id}")
        ...
        print(f"[AutoNN_CL] Continual Learning 프로세스 시작 완료 - PID: {pr_id}")
        print(f"[AutoNN_CL] START API 응답 전송 - 상태: started")
        return Response("started", status=status.HTTP_200_OK)
    except Exception as e:
        print(f"[AutoNN_CL GET/start] exception: {e}")
        info.status = "failed"
        info.save()
...
def resume(request):
    """
    API for project manager having autonn_cl resumed
    """
    params = request.query_params
    userid = params.get('user_id')
    project_id = params.get('project_id')
    ...
    try:
        ...
        if str(info.process_id) in PROCESSES:
            print(f"[AutoNN_CL GET/resume] Terminating previous process #{info.process_id}")
            zombie_process = PROCESSES.pop(str(info.process_id))
            if zombie_process.is_alive():
                zombie_process.terminate()
                zombie_process.join()
        ...
        print(f"[AutoNN_CL GET/resume] exception: {e}")
        info.status = "failed"
        info.save()
        return Response("failed", status=status.HTTP_400_BAD_REQUEST)
...
def stop(request):
    """
    API for project manager having autonn_cl clean
    """
    params = request.query_params
    userid = params.get('user_id')
    project_id = params.get('project_id')

    print(f"[AutoNN_CL] STOP API 수신 - User: {userid}, Project: {project_id}")
    ...
    try:
        ...
        print(f"[AutoNN_CL Get/stop] 프로세스 #{info.process_id} 종료 완료")
        print(f"[AutoNN_CL Get/stop] TODO: Delete all temporary files")
    except Info.DoesNotExist:
        print(f"[AutoNN_CL GET/stop] Requested process does not exist")

    print(f"[AutoNN_CL] STOP API 응답 전송 - 상태: success")
...
def status_request(request):
    """
    API for project manager polling autonn_cl status
    """
    params = request.query_params
    userid = params.get('user_id')
    project_id = params.get('project_id')
    
    print(f"[AutoNN_CL] STATUS_REQUEST API 수신 - User: {userid}, Project: {project_id}")

    try:
        info = Info.objects.get(userid=userid, project_id=project_id)
        if DEBUG:
            print('\n' + '='*15 + ' AutoNN_CL Heart Beat ' + '='*16)
            info.print()
    except Info.DoesNotExist:
        print(f"[AutoNN_CL] STATUS_REQUEST API 응답 전송 - 상태: ready (프로젝트 정보 없음)")
        return Response("ready", status=status.HTTP_204_NO_CONTENT)

    try:
        # 프로세스 ID가 없거나 PROCESSES에 없는 경우 현재 상태 반환
        if not info.process_id or str(info.process_id) not in PROCESSES:
            current_status = info.status or "ready"
            print(f"[AutoNN_CL] STATUS_REQUEST API 응답 전송 - 상태: {current_status} (프로세스 없음)")
            return Response(current_status, status=status.HTTP_208_ALREADY_REPORTED)
            
        # 프로세스가 살아있는지 확인
        process = PROCESSES[str(info.process_id)]
        if not process.is_alive():
            print(f"[AutoNN_CL] STATUS_REQUEST API 응답 전송 - 상태: {info.status} (프로세스 완료)")
            return Response(info.status, status=status.HTTP_208_ALREADY_REPORTED)
        
        # 프로세스 실행 중
        info.status = "running"
        info.save()
        print(f"[AutoNN_CL] STATUS_REQUEST API 응답 전송 - 상태: running")
        return Response("running", status=status.HTTP_200_OK)
    except Exception as e:
        print(f"[AutoNN_CL GET/status_request] exception: {e}")
        print(f"[AutoNN_CL] STATUS_REQUEST API 응답 전송 - 상태: failed")
        return Response("failed", status=status.HTTP_400_BAD_REQUEST)


def status_report(userid, project_id, status="success"):
    """
    Report status to project manager when the autonn_cl process ends
    """
    try:
        url = 'http://projectmanager:8085/status_report'
        headers = {'Content-Type': 'text/plain'}
        payload = {
            'container_id': "autonn_cl",
            'user_id': userid,
            'project_id': project_id,
            'status': status
        }
        response = requests.get(url, headers=headers, params=payload)
        print(f"[AutoNN_CL status_report] report = {status}, response = {response.status_code}")

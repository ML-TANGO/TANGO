...
''' [tenace] torchtune is currently alpha version '''
...
config_path = Path(f"{str(Path(__file__).parent.parent)}/cfg") # autonn_cl_core/tangochat/common/cfg
...
class ModelRecipe:
    ...
    def get_recipe(cls, model_type):
        ''' match - case statement is supported python >= 3.10 '''
        # match model_type:
...
class Transformer(nn.Module):
    ...
    ''' [tenace] distribute module is not supported in Python 3.8 '''

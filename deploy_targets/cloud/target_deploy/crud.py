...
def get_container_name(db: Session, user_input):
    try:
        return (
            ...
            .filter(
                models.Task.user_id == user_input["user_id"],
                models.Task.project_id == user_input["project_id"],
            )
            .first()
        )
...
def modify_container_id(db: Session, project_id, container_id):
    try:
        model = (
            db.query(models.Task).filter(models.Task.project_id == project_id).first()
        )
...
def modify_tasks_status(db: Session, user_data: dict, status: str):
    try:
        model = (
            db.query(models.Task)
            .filter(
                models.Task.user_id == user_data["user_id"],
                models.Task.project_id == user_data["project_id"],
            )
            .first()
        )
        model.status = status

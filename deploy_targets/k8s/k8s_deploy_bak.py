...
class K8SDeploy:
    ...
    def run(self):
        ...
        if ret == 0:
            ...
            run_kubernetes=deployment.KubeJob(job_name=self.m_current_userid, input_data=self.m_dep_work_dir, output_data=self.m_current_code_folder, nn_file=self.m_nn_file,
                                            ...
                                            nfs_ip=self.m_nfs_ip, nfs_path=self.m_nfs_path, image_name="localhost:8903/build/"+self.m_image_name, svc_port=self.m_dep_hostport)
    ...
    def make_dockerfile(self):
        ...
        if ret == 0:
            ...
            # Docker 이미지 빌드
            import subprocess
            subprocess.run(["docker", "build", "-f", "Dockerfile_new",  "-t", "localhost:8903/build/"+self.m_image_name, "."])                      
            subprocess.run(["docker", "push", "localhost:8903/build/"+self.m_image_name])
            subprocess.run(["docker", "rmi", "localhost:8903/build/"+self.m_image_name])                      
...
class MyHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if cmd == "start":
            ...
            if self.m_make_docker_file==0:
                print("test111")
                self.m_deploy_obj.make_dockerfile()

"""views module for deep_frameowrk
...
def login(request):
    ...
    try:
        ...
        user_info = authenticate(username=user_id, password=user_pw)
        ...
        if user_info is None:
            ...
            # 기존 토큰 정보가 있는 경우
                ...
                # for i in user_token_search:
                #     delete_result = delete_token(request, i.token)
                #
                # # 기존 토큰 정상 삭제된 경우
                ...
                # AbstractAccessToken(user_token_search).revoke()
                #
                # refresh = Oauth2ProviderRefreshtoken.objects.filter(user_id=user_info.id)
                #
                # AbstractRefreshToken(refresh).revoke()

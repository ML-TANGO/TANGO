  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (imgFile.ANNO_DATA !== null && imgFile.ANNO_DATA !== "" && !isTest) {
      if (req.body.OBJECT_TYPE === "S" || req.body.OBJECT_TYPE === "D") {
        // console.log(path.extname(imgFile.ANNO_DATA))
    ...
    // imgFile.IMG_ORG = _base64Encode(imgFile.FILE_PATH)
    imgFile.FILE_URL = url
    // console.log(imgFile.ANNO_DATA)
    res.json(imgFile)
  })
)
...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    // if (datasetInfo.AUTO_EPOCH > 0) option.queryId = "getDataUserTags"
    // else option.queryId = "getDataTags"
    option.queryId = "getDataTags"

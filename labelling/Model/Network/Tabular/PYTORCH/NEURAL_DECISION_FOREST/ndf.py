...
class CustomSetFeatureLayer(nn.Sequential):
    def __init__(self, dropout_rate=0., shallow=True, featureLen=5):
        super(CustomSetFeatureLayer, self).__init__()
        self.shallow = shallow
        self.featureLen = featureLen
        if shallow:
            self.add_module('linear', nn.Linear(self.featureLen, 1024))
        else:
            raise NotImplementedError

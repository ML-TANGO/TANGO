...
from Network.Tabular.SCIKIT.kNN_REG import predict, model
    ...
    try:
        ...
        if output["SUCCESS"]:
            ...
            gs = GridSearch(
                estimator=reg,
                param_grid={
                    "n_neighbors":range(1, (int(param["n_neighbors"]) if "n_neighbors" in param else 10)+1, 1),
                    "weights":[str(param["weights"])] if "weights" in param else ["uniform"],
                    "leaf_size":[int(param["leaf_size"])] if "leaf_size" in param else [30],
                    "algorithm":[str(param["algorithm"])] if "algorithm" in param else ["auto"],
                    "n_jobs":[-1]
                },
                param=param,
                mode='reg'

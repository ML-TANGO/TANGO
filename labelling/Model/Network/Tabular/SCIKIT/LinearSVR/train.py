...
from Network.Tabular.SCIKIT.LinearSVR import predict, model
    ...
    try:
        ...
        if output["SUCCESS"]:
            ...
            gs = GridSearch(
                estimator=reg,
                param_grid={
                    "max_iter":range(1, (int(param["max_iter"]) if "max_iter" in param else 1000)+1, 1),
                    "C":[float(param["C"])] if "C" in param else [1],                    
                    "dual":[bool(param["dual"])] if "dual" in param else [True],
                    "loss":[str(param["loss"])] if "loss" in param else ["epsilon_insensitive"],
                    "random_state":[42]  
                },
                param=param,
                mode="reg"

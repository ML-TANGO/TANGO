...
def check_git_status():
    ...
    try:
        ...
        s += f'failed to check git status because {e}'
        logger.warn(s)
...
def check_img_size(img_size, s=32):
    ...
    if new_size != img_size:
        logger.warn('WARNING: --img-size %g must be multiple of max stride %g, updating to %g' % (img_size, s, new_size))
    return new_size
...
def check_imshow():
    ...
    try:
        ...
        logger.warn(f'WARNING: Environment does not support cv2.imshow() or PIL Image.show() image displays\n{e}')
        return False
...
def strip_optimizer(f='best.pt', s=''):  # from utils.general import *; strip_optimizer()
    ...
    logger.info(f"Model Exporter: Optimizer stripped from {f},{(' saved as %s,' % s) if s else ''} {mb:.1f}MB")

...
def create_dataloader_v9(path,
    ...
    return loader(dataset,
                  ...
                  sampler=sampler,
                  pin_memory=True,
                  collate_fn=LoadImagesAndLabels_v9.collate_fn4 if quad else LoadImagesAndLabels_v9.collate_fn,
...
class LoadImagesAndLabels_v9(Dataset):
    ...
    def __init__(self,
        ...
        try:
            ...
            for p in path if isinstance(path, list) else [path]:
                ...
                if p.is_dir():  # dir
                    ...
                    raise FileNotFoundError(f'{colorstr(prefix)} {p} does not exist')
            self.im_files = sorted(x.replace('/', os.sep) for x in f if x.split('.')[-1].lower() in img_formats)
            # self.img_files = sorted([x for x in f if x.suffix[1:].lower() in IMG_FORMATS])  # pathlib
            assert self.im_files, f'{colorstr(prefix)} No images found'
        except Exception as e:
            raise Exception(f'{colorstr(prefix)} Error loading data from {path}: {e}') from e
        ...
        try:
            ...
            assert cache['hash'] == get_hash_v9(self.label_files + self.im_files)  # identical hash
        except Exception:
    ...
    def __getitem__(self, index):
        ...
        # logger.info(f'getting item no.{index}')
        hyp = self.hyp
        ...
        if mosaic:
            ...
            if labels.size:  # normalized xywh to pixel xyxy format
                labels[:, 1:] = xywhn2xyxy_v9(labels[:, 1:], ratio[0] * w, ratio[1] * h, padw=pad[0], padh=pad[1])

            if self.augment:
                img, labels = random_perspective_v9(img,
                                                 labels,
                                                 degrees=hyp['degrees'],
        ...
        if self.augment:
            ...
            # HSV color-space
            augment_hsv_v9(img, hgain=hyp['hsv_h'], sgain=hyp['hsv_s'], vgain=hyp['hsv_v'])
    ...
    def load_mosaic(self, index):
        ...
        # Augment
        img4, labels4, segments4 = copy_paste_v9(img4, labels4, segments4, p=self.hyp['copy_paste'])
        img4, labels4 = random_perspective_v9(img4,
                                           labels4,
                                           segments4,
    ...
    def load_mosaic9(self, index):
        ...
        img9, labels9 = random_perspective_v9(img9,
                                           labels9,
                                           segments9,
                                           degrees=self.hyp['degrees'],
                                           translate=self.hyp['translate'],
...
class LoadDetectionData(Dataset):  # for exporting
...
def verify_image_label(args):
    ...
    try:
        ...
        im = Image.open(im_file)
        im.verify()  # PIL verify
        shape = exif_size_v9(im)  # image size
        assert (shape[0] > 9) & (shape[1] > 9), f'image size {shape} <10 pixels'
        assert im.format.lower() in img_formats, f'invalid image format {im.format}'
...
def augment_hsv_v9(im, hgain=0.5, sgain=0.5, vgain=0.5):
...
def copy_paste_v9(im, labels, segments, p=0.5):
...
def random_perspective_v9(im,
    ...
    if n:
        ...
        # filter candidates
        i = box_candidates_v9(box1=targets[:, 1:5].T * s, box2=new.T, area_thr=0.01 if use_segments else 0.10)
        targets = targets[i]
        targets[:, 1:5] = new[i]
...
def box_candidates_v9(box1, box2, wh_thr=2, ar_thr=100, area_thr=0.1, eps=1e-16):  # box1(4,n), box2(4,n)
    # Compute candidate boxes: box1 before augment, box2 after augment, wh_thr (pixels), aspect_ratio_thr, area_ratio
    w1, h1 = box1[2] - box1[0], box1[3] - box1[1]
    w2, h2 = box2[2] - box2[0], box2[3] - box2[1]
    ar = np.maximum(w2 / (h2 + eps), h2 / (w2 + eps))  # aspect ratio
...
class Albumentations_v9:
    ...
    def __init__(self, size=640):
        ...
        try:
            ...
            logger.warning(f'{prefix}ImportError')
        except Exception as e:

...
def kmean_anchors(path='./data/coco.yaml', n=9, img_size=640, thr=4.0, gen=1000, verbose=True):
    ...
    def print_results(k):
        ...
        logger.info(f'{prefix}n={n}, img_size={img_size}, metric_all={x.mean():.3f}/{best.mean():.3f}-mean/best, '
              f'past_thr={x[x > thr].mean():.3f}-mean: ')
        for i, x in enumerate(k):
            logger.info('%i,%i' % (round(x[0]), round(x[1])))  # use in *.cfg
        return k
    ...
    assert len(k) == n, logger.info(f'{prefix}ERROR: scipy.cluster.vq.kmeans requested {n} points but returned only {len(k)}')
    k *= s

...
def init_seeds(seed=0, deterministic=False, version='v7'):
    # Initialize random number generator (RNG) seeds
    random.seed(seed)
    np.random.seed(seed)
    init_torch_seeds(seed, deterministic, version)
...
def check_dataset(data_dict):
    # Checks: dataset.yaml rule
    for k in 'train', 'val', 'names':
        assert k in data_dict, f"dataset.yaml '{k}:' field missing"
    if isinstance(data_dict['names'], (list, tuple)): # old array format
        data_dict['names'] = dict(enumerate(data_dict['names'])) # convert to dict
    assert all(isinstance(k, int) for k in data_dict['names'].keys()), 'data.yaml names keys must be integers, i.e. 2: car'
    if not 'nc' in data_dict:
...
### for yolov9, should be merged with original one(v7) at some point
def bbox_iou_v9(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, eps=1e-7):
    """
...
def non_max_suppression_v9(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        # Apply constraints
        # x[((x[:, 2:4] < min_wh) | (x[:, 2:4] > max_wh)).any(1), 4] = 0  # width-height
        x = x.T[xc[xi]]  # confidence filtering
        # Cat apriori labels if autolabelling
        if labels and len(labels[xi]):
            lb = labels[xi]

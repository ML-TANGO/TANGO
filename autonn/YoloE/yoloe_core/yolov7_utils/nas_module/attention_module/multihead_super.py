...
class AttentionSuper(nn.Module):
    def __init__(self, super_embed_dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., normalization = False, relative_position = True,
    ...
    def set_sample_config(self, sample_q_embed_dim=None, sample_num_heads=None, sample_in_embed_dim=None):
        ...
        if False:
            ...
            self.sample_qk_embed_dim = 64
            self.sample_scale = (self.sample_qk_embed_dim) ** -0.5

        self.qkv.set_sample_config(sample_in_dim=sample_in_embed_dim, sample_out_dim=3*self.sample_qk_embed_dim * self.sample_num_heads)
        self.proj.set_sample_config(sample_in_dim=self.sample_qk_embed_dim * self.sample_num_heads, sample_out_dim=sample_in_embed_dim)
        if self.relative_position:

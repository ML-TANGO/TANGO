...
from utils.v9_utils import dist2bbox, make_anchors
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect8, DualDDetect)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolo.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        # Build strides, anchors
        m = self.model[-1]  # DualDDetect() or Detect8()
        if isinstance(m, Detect8):
            s = 256  # 2x min stride
        ...
        # Print summary (layers, Params, Grads, FLOPS)
        self.info()
        logger.info('')
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    logger.info('\n%3s%28s%10s  %-20s%-30s' % ('#', 'from', 'params', 'module', 'arguments'))
    logger.info('-'*100)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        logger.info('%3s%28s%10.0f  %-20s%-30s' % (i, f, np, t, args))  # print

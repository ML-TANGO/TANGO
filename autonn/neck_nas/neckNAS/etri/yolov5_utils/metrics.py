...
def ap_per_class(tp, conf, pred_cls, target_cls, plot=False, save_dir='.',
    ...
    if plot:
        plot_pr_curve(
            px, py, ap,
            Path(save_dir) / 'PR_curve.png', names)
        plot_mc_curve(
            px, f1,
            Path(save_dir) / 'F1_curve.png', names, ylabel='F1')
        plot_mc_curve(
            px, p,
            Path(save_dir) / 'P_curve.png', names, ylabel='Precision')
        plot_mc_curve(
            px, r,
            Path(save_dir) / 'R_curve.png', names, ylabel='Recall')
    ...
    def plot(self, normalize=True, save_dir='', names=()):
        try:
            ...
            with warnings.catch_warnings():
                # suppress empty matrix RuntimeWarning:
                ...
                sn.heatmap(array,
                           ...
                           yticklabels=names + ['background FN']
                                if labels else "auto").set_facecolor((1, 1, 1))
...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False,
    ...
    if CIoU or DIoU or GIoU:
        ...
        if CIoU or DIoU:
            ...
            if CIoU:
                # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/

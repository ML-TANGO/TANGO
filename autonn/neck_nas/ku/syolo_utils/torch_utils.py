...
def select_device(device='', batch_size=None):
    ...
    if device and not cpu_request:  # if device requested other than 'cpu'
        ...
        assert torch.cuda.is_available(), 'CUDA unavailable, \
            invalid device %s requested' % device  # check availablity
...
def fuse_conv_and_bn(conv, bn):
    ...
    with torch.no_grad():
        ...
        b_bn = bn.bias - \
            bn.weight.mul(bn.running_mean).div(torch.sqrt(bn.running_var +
                                                          bn.eps))
...
def model_info(model, verbose=False):
    ...
    try:  # FLOPS
        ...
        flops = profile(deepcopy(model), inputs=(torch.zeros(1, 3, 64, 64),),
...
class ModelEMA:
    ...
    Keep a moving average of everything in the model state_dict
    (parameters and buffers).
    This is intended to allow functionality like
    https://www.tensorflow.org/api_docs/python/tf/train/ExponentialMovingAverage
    A smoothed version of the weights is necessary for
    some training schemes to perform well.
    This class is sensitive where it is initialized
    in the sequence of model init,
    GPU assignment and distributed training wrappers.
    """

    def __init__(self, model, decay=0.9999, updates=0):
        # Create EMA
        self.ema = deepcopy(model.module if is_parallel(model)
                            else model).eval()  # FP32 EMA
    ...
    def update(self, model):
        ...
        with torch.no_grad():
            ...
            msd = model.module.state_dict() if is_parallel(model) \
                else model.state_dict()  # model state_dict

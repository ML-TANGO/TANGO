...
from syolo_utils.general import (
    labels_to_class_weights, labels_to_image_weights, check_anchors, plot_images, 
    fitness, strip_optimizer, plot_results, plot_labels)
from syolo_utils.torch_utils import init_seeds, ModelEMA, intersect_dicts, is_parallel
...
class ProxylessDetTrainer(BaseOneShotTrainer):
    ...
    def __init__(self, model, args, hyp, loss, imgsz, imgsz_test, train, dataset, test, writer, optimizer, lf, scheduler,
        ...
        self.nc, self.names = (1, ['item']) if args.single_cls else (int(data_dict['nc']), data_dict['names'])  # number classes, names
        assert len(self.names) == self.nc, '%g names found for nc=%g dataset in %s' % (len(names), nc, opt.data)  # check
        ...
        # ScaledYOLOv4 trainer
        self.log_dir = Path(self.writer.logdir)
        self.wdir = str(self.log_dir / 'weights') + os.sep   # weights directory
        ...
        self.total_batch_size = args.total_batch_size # for DDP mode
        ...
        if self.rank in [-1, 0]:
            ...
            if not self.args.noautoanchor:
                check_anchors(self.dataset, model=self.model, thr=self.hyp['anchor_t'], imgsz=self.image_size)
        ...
        if self.rank in [0, -1]:
            print('Image sizes %g train, %g test' % (self.image_size, self.image_size_test))
    ...
    def _train_one_epoch(self, epoch):
        ...
        # save current architecture prob for history
        probs_history = {}
        export_architecture = {}
        ...
        for i, (imgs, targets, paths, _) in pbar:   # batch -------------------------------------------------------------
            ...
            if self.args.multi_scale:
                sz = random.randrange(self.image_size * 0.5, self.image_size * 0.5 + self.gs) // self.gs * self.gs  # size
                sf = sz / max(imgs.shape[2:])   # scale factor
                if sf != 1:
                    ns = [math.ceil(x * sf / self.gs) * self.gs for x in imgs.shape[2:]]    # new shape (stretched to gs-multiple)
                    imgs = F.interpolate(imgs, size=ns, mode='bilinear', align_corners=False)
                    # imgs_test = F.interpolate(imgs_test, size=ns, mode='bilinear', align_corners=False)
            ...
            # if ni < self.nw:
                ...
                try:    # len(test_loader) < len(train_loader)
                    imgs_test, targets_test, _, _ = next(self.test_loader_iterator)
                except StopIteration:
                    self.test_loader_iterator = iter(self.test_loader)
                    imgs_test, targets_test, _, _ = next(self.test_loader_iterator)
                imgs_test, targets_test = imgs_test.to(self.device, non_blocking=True).float() / 255.0, targets_test.to(self.device)
                loss, _ = self._loss_and_items_for_arch_update(imgs_test, targets_test, self.cuda)
                loss.backward()
                for _, module in self.nas_modules:
            ...
            for module_name, module in self.nas_modules:
                module.resample()
            loss, loss_items = self._loss_and_items_for_weight_update(imgs, targets.to(self.device), self.cuda)
            loss.backward()
            ...
            if self.rank in [-1, 0]:
                ...
                mem = '%.3gG' % (torch.cuda.memory_reserved(device=self.device) / 1E9 if torch.cuda.is_available() else 0)  # (GB)
                s = ('%10s' * 2 + '%10.4g' * 6) % (
                ...
                # Plot
                if ni < 3:
                    f = str(self.log_dir / ('train_batch%g.jpg' % ni))  # filename
                    result = plot_images(images=imgs, targets=targets, paths=paths, fname=f)
                    if self.writer and result is not None:
                        self.writer.add_image(f, result, dataformats='HWC', global_step=epoch)
                        # self.writer.add_graph(self.model, imgs)  # add model to tensorboard
        ...
        if self.rank in [-1, 0]:
            ...
            if not self.args.notest or final_epoch:  # Calculate mAP
                ...
                with fixed_arch(arch):
                    self.model_test = SearchYolov4(cfg=self.args.cfg,
                                                    names=self.names,
                                                    hyp=self.hyp,
                                                    ch=3,
                                                    nc=self.nc).to(self.device)
                ckpt_ema = self.ema.ema.module.state_dict().copy() if is_parallel(self.ema.ema) else self.ema.ema.state_dict().copy()
                ...
                exclude = []
                state_dict = intersect_dicts(ckpt_ema, self.model_test.state_dict(), exclude=exclude)   # intersect
                self.model_test.load_state_dict(state_dict, strict=False)   # load

                self.results, self.maps, times = test.test(self.args.data,
                                                          batch_size=self.batch_size,
                                                          imgsz=self.image_size_test,
                                                          save_json=final_epoch and self.args.data.endswith(os.sep + 'coco.yaml'),
            ...
            if len(self.args.name) and self.args.bucket:
                os.system('gsutil cp %s gs://%s/results/results%s.txt' % (self.results_file, self.args.bucket, self.args.name))
            ...
            # Update best mAP
            fi = fitness(np.array(self.results).reshape(1, -1))  # fitness_i = weighted combination of [P, R, mAP, F1]
            ...
            if save:
                ...
                if epoch >= (self.epochs-30):
                    torch.save(ckpt, self.last.replace('.pt','_{:03d}.pt'.format(epoch)))
                if self.best_fitness == fi:
                    torch.save(ckpt, self.best)
                del ckpt
        # end epoch ----------------------------------------------------------------------------------------------------

    def _loss_and_items_for_arch_update(self, imgs, targets, cuda):
        ''' return loss and loss_items for architecture parameter update '''
        pred = self.model(imgs)
        loss, loss_items = self.loss(pred, targets, self.model) # scaled by batch_size
        if self.rank != -1:
            loss *= self.args.world_size    # gradient averaged between devices in DDP mode
        return loss, loss_items

    def _loss_and_items_for_weight_update(self, imgs, targets, cuda):
    ...
    def fit(self):
        ...
        if self.rank in [-1, 0]:
            # Strip optimizers
            n = ('_' if len(self.args.name) and not self.args.name.isnumeric() else '') + self.args.name
            ...
            for f1, f2 in zip([self.wdir + 'last.pt', self.wdir + 'best.pt', 'results.txt'], [flast, fbest, fresults]):
                if os.path.exists(f1):
                    ...
                    os.system('gsutil cp %s gs://%s/weights' % (f2, self.args.bucket)) if self.args.bucket and ispt else None  # upload
            # Finish
            plot_results(save_dir=self.log_dir)  # save as results.png
            print('%g epochs completed in %.3f hours.\n' % (self.epochs - self.start_epoch + 1, (time.time() - t0) / 3600))

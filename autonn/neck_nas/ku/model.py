...
from models.ops_syolov4 import *
# import models.ops_syolov4 as ops_syolov4
from models.experimental import MixConv2d, CrossConv, C3
from syolo_utils.general import check_anchor_order, make_divisible, check_file
from syolo_utils.torch_utils import (
    time_synchronized, fuse_conv_and_bn, model_info, scale_img, intersect_dicts)
...
class SearchYolov4(nn.Module):
    def __init__(self, cfg='yolov4-p5.yaml', names=None, hyp=None, weights=None, ch=3, nc=None):
        ...
        self.model, self.save = self._parse_model(deepcopy(self.yaml), ch=[ch])   # model, savelist, ch_out
    ...
    def _parse_model(self, d, ch):  # model_dict, input_channels(3)
        ...
        for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
            ...
            if m in [nn.Conv2d, Conv, ActConv, Bottleneck, MBottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP, BottleneckCSP2, MBottleneckCSP2, SPPCSP, VoVCSP, C3]:
                c1, c2 = ch[f], args[0]
                ...
                args = [c1, c2, *args[1:]]
                if m in [BottleneckCSP, BottleneckCSP2, MBottleneckCSP2, SPPCSP, VoVCSP, C3]: 
                    args.insert(2, n)
                    n = 1
            ...
            if m is MBottleneckCSP2:
                ...
                m_ = MBottleneckCSP2(layer_op, op_candidates)
            else:
    ...
    def init_model(self):
        ...
        if pretrained:
            temp = dict()
            ckpt = torch.load(self.weights, map_location=temp)
            # ckpt = torch.load(self.weights, map_location=device)
            exclude = ['anchor'] if self.yaml else [] # exclude keys
            state_dict = ckpt['model'].float().state_dict() # to FP32
            for key in list(state_dict.keys()):   # to match keys
                ks = key.split('.')
                new_key = ks[1:]
            ...
            del ckpt, temp
            print('Transferred %g/%g items from %s' % (len(state_dict), len(self.model.state_dict()), self.weights))
...
def project_delete(request):
    ...
    dir_path = os.path.join(root_path, "shared/common/{0}/{1}".format(str(request.user),
                                                                      str(request.data['id'])))

    # 디렉토리가 존재하는 경우 - project_info.yaml 파일이 존재하는 디렉토리 제거
...
def project_info(request):
    ...
    # Project id로 검색
    queryset = Project.objects.filter(id=request.data['id'], create_user=request.user)
    project_data = list(queryset.values())
    ...
    target = {'target': ''}
    if target_id != '':
        target_queryset = Target.objects.filter(id=int(target_id))
        target_data = list(target_queryset.values())
...
def project_update(request):
    ...
    try:
        ...
        # 타겟 정보 수신
        target_data = Target.objects.get(id=int(target))
        ...
        queryset.deploy_weight_level = deploy_weight_level if target_data.info != 'ondevice' else ''
        queryset.deploy_precision_level = deploy_precision_level if target_data.info != 'ondevice' else ''
        queryset.deploy_processing_lib = deploy_processing_lib if target_data.info != 'ondevice' else ''
        queryset.deploy_user_edit = deploy_user_edit if target_data.info != 'ondevice' else ''
        queryset.deploy_input_method = deploy_input_method if target_data.info != 'ondevice' else ''
        queryset.deploy_input_data_path = deploy_input_data_path if target_data.info != 'ondevice' else ''
        queryset.deploy_output_method = deploy_output_method if target_data.info != 'ondevice' else ''
        queryset.container = ''
        queryset.container_status = ''
        queryset.yaml_make = True
        # 프로젝트 - 신경망 생성 정보 저장
        queryset.save()

        project_info_content = ""
        if target_data.info != 'ondevice':
            # project_info.yaml
            project_info_content += "# common\n" \
                                   ...
                                   "# user_editing : {19}\n".format(task_type,
                                                                    ...
                                                                    target_data.engine,
                                                                    target_data.host_ip,
                                                                    target_data.host_port,
                                                                    target_data.host_service_port,
                                                                    autonn_dataset_file,
                                                                    autonn_basemodel,
                                                                    nas_type,
                                                                    deploy_weight_level,

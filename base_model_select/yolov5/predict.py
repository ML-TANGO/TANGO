...
def save_one_txt(predn, save_conf, shape, file):
    ...
    for *xyxy, conf, cls in predn.tolist():
        ...
        print("save_one_txt", str(file))
        with open(file, 'a') as f:
            f.write(('%g ' * len(line)).rstrip() % line + '\n')


def save_one_json(predn, jdict, path, class_map):
    # json 파일에 모든 결과를 하나로 모아서 저장
...
def run(
    ...
    if not training:
        ...
        task = task if task in ('train', 'val', 'test', 'evaluate') else 'val'  # path to train/val/test images
    ...
    print("print_results", seen, mp, mr, map50, map)
    # seen: sample 개수
    # mp: Mean Precision
    # mr: Mean Recall
    # map50: map0.5
    ...
    if not training:
        ...
        print("print_s_peed", t)
        LOGGER.info(f'Speed: %.1fms pre-process, %.1fms inference, %.1fms NMS per image at shape {shape}' % t)
        # t: (3)tuple contains pre-process, inferernce, NMS per images at shape 
...
def parse_opt(docker):
    ...
    parser.add_argument('--task', default='val', help='train, val, test, speed, evaluate or study')
...
def main(opt, userid="", project_id=""):
    ...
    if opt.task in ('train', 'val', 'test'):  # run normally
        ...
        if opt.task == 'speed':  # speed benchmarks
            ...
            total_evaluate = np.zeros(5)
            total_time = np.zeros(5)
            total_map = np.zeros(5)
            total_model = []
            index = 0
            # python val.py --task evaluate --data coco.yaml --batch 1 --weights yolov5n.pt yolov5s.pt...
            opt.conf_thres, opt.iou_thres, opt.save_json = 0.25, 0.45, False
            print("here is the data", opt.data)
            
            for opt.weights in ['yolov5n.pt', 'yolov5s.pt', 'yolov5m.pt', 'yolov5l.pt', 'yolov5x.pt']:
                # f = f'evlaluate_{Path(opt.data).stem}_{Path(opt.weights).stem}.txt'
                r, _, t = run(**vars(opt), plots=False)
                print(opt)
                print("r is", r[3])
                print("t is", t)
                
                total_model.append(opt.weights)
                total_time[index] = sum(list(t))
                total_map[index] = r[3]
                # TODO: NEED TO CHANGE THE FORMULA LATER
                total_evaluate[index] =total_map[index] / total_time[index]
                index = index + 1 
            
            max_index = np.argmax(total_evaluate)
            print("best result is in ", total_model[max_index], "with the value of ", total_time[max_index], total_map[max_index])
            print('           ', ' yolov5n.pt', ' yolov5s.pt', ' yolov5m.pt', ' yolov5l.pt', ' yolov5x.pt')
            print('total_time', total_time)
            print('total_map ', total_map)
            print('total_eval', total_evaluate)
            ...
            opt.task = 'val'
            opt.weights = total_model[max_index]
            if opt.conf_thres > 0.001:  # https://github.com/ultralytics/yolov5/issues/1466
                LOGGER.info(emojis(f'WARNING: confidence threshold {opt.conf_thres} > 0.001 produces invalid results ⚠️'))
            run(**vars(opt))
            """
